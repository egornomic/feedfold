...
describe("feed subscription workflow", () => {
  ...
  it.each(["web", "desktop"] as const)(
    "delivers new and cached feeds, leaves paused feeds idle, and publishes web selections through %s",
    async (transport) => {
      ...
      await expect(
        createFeed({ sourceKind: "published", feedUrl: fresh.feedUrl }),
      ).rejects.toThrow();
      expect(
      ...
      expect(deliveredTitles).toHaveLength(1);
      if (transport === "web") {
        const { webFeedService: _webFeedService, ...servicesWithoutBrowser } = services;
        const appWithoutBrowser = await createApp({
          ...servicesWithoutBrowser,
