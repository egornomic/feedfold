...
describe("passkey authentication", () => {
  it.each(["open", "invite"] as const)(
    "creates an account in %s mode with a discoverable passkey and signs in without a username or password",
    async (registrationMode) => {
      ...
      expect(database.feeds.listFeeds(account?.id ?? 0)).toEqual([]);
      await cdp.send("WebAuthn.addVirtualAuthenticator", {
        options: {
          protocol: "ctap2",
          transport: "usb",
